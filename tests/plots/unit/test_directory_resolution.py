...
def test_directory_resolution(tmp_examples_mlvern, sample_df):
    # don't pass mlvern_dir -> basic_eda should detect examples/.mlvern in cwd
    res = basic_eda(sample_df, output_dir="unused", mlvern_dir=None)
    # ensure plots were created under the examples/.mlvern directory
    plots_dir = os.path.join(tmp_examples_mlvern, "plots", "eda")
    assert os.path.isdir(plots_dir)
    # report present
    assert os.path.isfile(os.path.join(tmp_examples_mlvern, "reports", "eda_report.json"))
